...
function ENT:Grow()
	...
	elseif (strType == "melon") then
		local num = 1
		if (ply:HasUnlock("Adept_Farmer")) then num = num + math.random(0, 1) end
		if (ply:HasUnlock("Expert_Farmer")) then num = num + math.random(0, 2) end
...
function ENT:OnRemove()
	if (self.Entity.Fade != true) then
		if (strType != "tree") then self.Entity:GetOwner():SetNWInt("plants", self.Entity:GetOwner():GetNWInt("plants") - 1) end	
