...
function SPropProtection.LoadBuddies(ply)
	...
	if (PData != "") then
		for k, v in pairs(string.Explode(";", PData)) do
			...
			if (String != "") then table.insert(SPropProtection[ply:SteamID()], String) end
		end
	end
end
...
function SPropProtection.PlayerMakePropOwner(ply, ent)
	...
	ent:SetNetworkedString("TribeID", ply:Team())
	...
	for k, v in pairs(Players) do
		if (v and v:IsValid() and v != ply) then
	        if (SPropProtection["Props"][ent:EntIndex()][1] == v:SteamID()) then
				if (ply:Team() == v:Team() and ply:Team() != 1 and GAMEMODE.FindTribeByID(ply:Team()).Password != false) then return true end
...
function SPropProtection.PlayerCanTouch(ply, ent)
	if (tonumber(SPropProtection["Config"]["toggle"]) == 0 or ent:GetClass() == "worldspawn") then return true end
	...
	if (string.find(ent:GetClass(), "stone_") == 1 or string.find(ent:GetClass(), "rock_") == 1 or string.find(ent:GetClass(), "stargate_") == 0 or string.find(ent:GetClass(), "dhd_") == 0 or ent:GetClass() == "flag" or ent:GetClass() == "item") then
		...
		if (ply:GetActiveWeapon():GetClass() != "weapon_physgun" and ply:GetActiveWeapon():GetClass() != "gmod_tool") then return true end
	end
	...
	if (game.GetMap() == "gm_construct" and ent:GetNetworkedString("Owner") == "World") then return true end
	return false
end
...
function SPropProtection.RemoveTribeProps(TribeID)
	for k, v in pairs(SPropProtection["Props"]) do
		if (v[2]:IsValid() and tonumber(v[2]:GetNetworkedString("TribeID", "1")) == TribeID) then
			v[2]:Remove()
			SPropProtection["Props"][k] = nil
		end
	end
...
function SPropProtection.CheckForEmptyTribes()
	for name, t in pairs(GAMEMODE.Tribes) do
		if (team.NumPlayers(t.id) == 0) then
			timer.Create("SPropProtection.RemoveTribeProps: " .. t.id, tonumber(SPropProtection["Config"]["delay"]), 1, SPropProtection.RemoveTribeProps, ply:Team())
		end
	end
end
...
function SPropProtection.TribePP(ply) -- Set or unset player props for tribe
	...
	for k, v in pairs(SPropProtection["Props"]) do
		if (v[1] == ply:SteamID() and v[2]:IsValid()) then
			...
			local TimerName = "SPropProtection.RemoveTribeProps: " .. ID
...
function SPropProtection.PhysGravGunPickup(ply, ent)
	...
	if (!ent:IsValid() or !SPropProtection.PlayerCanTouch(ply, ent)) then return false end
end
