	...
	if file.Exists("GMStranded/Saves/"..ply:UniqueID()..".txt") then
		...
		if tbl["resources"] then
			for k,v in pairs(tbl["resources"]) do
				local name = string.Explode("_", k)
				local s = ""
				for id,str in pairs(name) do
					s = s..string.upper(string.sub(str, 0, 1)) .. string.sub(str, 2, string.len(str))
					if (id != #name) then
						s = s .. "_"
					end
				end
...
function GM.SaveCharacter(ply,cmd,args)
	/* Function Edited by Robotboy655 specially for SS.net*/
	...
	for id, str in pairs({"ar2", "smg1", "pistol", "buckshot", "357", "grenade", "alyxgun", "xbowbolt"}) do
		tbl["ammo"][str] = ply:GetAmmoCount(str)
	end
